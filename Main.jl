...
4.The simulator has the same functonality as Ripes. Reasons : 
    i] The Address starts from 0 and goes till 4096.
   ii] Each memory unit holds maximum of 1 byte i.e. 8bits 
  iii] The Instructions in the files are first encoded into 32 binary string and placed in the memory from the first address bit 
...
5.Helper Functions : 
    i] To display the memory of the processor use the function (the function displays 4 memory units in a row at a time in hexadecimal value) 
                *Note : It displays the memory in reverse manner. i.e. address 0 at bottom at address 4096 at top 
                        = >           show(processor,starting_row,ending_row)                 
                            where : processor = object of processor created
                                    starting_row  = Lower address
                                    ending_row = Upper address
                        = > Example ( paste the following code in quotes[".."] below after run command ) : "show(sim,513,530)"    #For Displaying Data Segment of Core 1

6.Memory Partitions Desicions : 
    i] The Data segment for both cores are fixed , i.e 1024 bytes for each core data segment. One core cannot access the data segment of other core
   ...
   ii] Data Segment : 
        For Core 1 , data segment starts from address 2048 to 3071           (starting_row = 513)        =======>      show(sim,513,535)
        For Core 2 , data segment starts from address 3072 to 4096           (starting_row = 769)        =======>      show(sim,769,790)
  iii] Text Segment ( Instructions ):
        For Core 1 ,  Instructions are stored from address 0                 (starting_row = 1)        =======>      show(sim,1,20)
        For Core 2 ,  Instructions are stored after instructions of core1    (starting_row = undefined)        =======>      show(sim,1,20)
=#
...
#Initialinz a processor object ( Check "Processor_Core_Init.jl" File)
sim = processor_Init()  
